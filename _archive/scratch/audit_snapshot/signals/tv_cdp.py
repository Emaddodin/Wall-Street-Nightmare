...
class TradingViewCDP:
    ...
    def _target(self) -> dict:
        ...
        # Chrome hands /json back in most-recently-activated order, not in the
        # order the tabs were opened. Both chart tabs carry the same saved
        # layout URL, so position was the only thing telling them apart -- and
        # Page.bringToFront, which the screenshot path must call to get a
        # composited frame, reorders that list every time a picture is taken.
        # The two tabs then swapped identities underneath the services: the
        # book began reading the tab the scout was walking, and the scout drove
        # the tab the book meant to hold still. Sorting on the target id, which
        # is fixed for the life of a tab, makes window 0 name the same tab in
        # every process no matter which one was last brought forward.
...
class CDPSignalSource:
    ...
    def __init__(self, study_match: str = "TESLA", host: str = DEFAULT_HOST,
        ...
        self.cdp = TradingViewCDP(host, port)
