...
def walk_order(coins: list[str], perched: list[str],
               exp_rank: dict[str, float]) -> list[str]:
    """The sweep order: ripe coins circle first, then the rest by how much
    they are EXPANDING -- measured, an expansion of 2x carries a 36.6%
    chance of a 5% move in eight bars against 4.2% quiet, so the eyesight
    goes where the move is brewing."""
    rest = [c for c in coins if c not in perched]
    rest.sort(key=lambda c: (-(exp_rank.get(c, 0.0) or 0.0), c))
...
def main() -> int:
    ...
    while True:
        ...
        coins = walk_order(coins, perched, exp_rank)
        for sym in coins:
