...
def scalper_state() -> dict | None:
    path = f"{_BOT}/data/relapse_scalper_state.json"
    try:
        if os.path.exists(path):
            with open(path, "r", encoding="utf-8") as f:
                return json.load(f)
    except Exception as e:
        log.warning("could not read scalper state: %s", e)
    return None
...
`;
 ...
 if(sc){
  ...
  const curEq=Number(sc.equity||65);
  if(geq){
   geq.textContent='$'+curEq.toFixed(2);
   geq.className='hero'+(curEq>65?' green':(curEq<65?' red':''));
  }
  ...
  const pnlVal=Number(sc.pnl_dollar||0);
  const pnlPctVal=Number(sc.pnl_pct||0);
  const pnlSign=pnlVal>=0?'+':'';
  set('gold_eqsub',`target $10,000 &middot; pnl ${pnlSign}$${pnlVal.toFixed(2)} (${pnlSign}${pnlPctVal.toFixed(2)}%) &middot; 100x lev`);
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/" or p == "/index.html":
            q = parse_qs(urlparse(self.path).query)
            token_arg = (q.get("t") or [""])[0]
            if secrets.compare_digest(token_arg, TOKEN):
                self._send_html(PAGE, cookie=new_session())
                return
            if session_ok(self._cookie()):
            ...
            self._send_html(LOGIN.replace("__ERR__", ""))
            return
        if False:
            pass
            d = PAGE.encode()
