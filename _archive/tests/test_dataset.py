...
def test_pine_windows_are_scaled_for_the_one_hour_htf():
    """The indicator was built for a 4h HTF; its wall-clock windows are
    the 4h-era values x4 now that the HTF is 1h. A revert to a 4h-sized
    window would quietly re-shrink every signal's memory by 4x."""
    pine = (ROOT / "pine" / "TBT_Sniper.pine").read_text()
    for needle in ("L_FAST = 100", "L_SLOW = 1120",
                   "t45Max    = input.int(320,",
                   "winGood = input.int(140,",
                   "comboCooldown = input.int(20,",
                   "htfLen = 220",
                   "snipWin   = input.int(40,",
                   "bnkVolLen = 80",
                   "bnkKeep = 32",
                   "trFast = 84", "trSlow = 220",
                   "adxLen = 56",
                   "maFastN = input.int(84,", "maSlowN = input.int(220,",
                   "fibBars = input.int(12,",
                   "max_bars_back=1500",
                   "winPeer = input.int(5,",
                   "winExc  = input.int(10,"):
