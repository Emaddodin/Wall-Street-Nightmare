...
def test_the_stop_is_always_inside_the_liquidation_line():
    ...
    i = src.index("def sym_lev(")
    body = src[i:i + 2200]
    # The ceiling follows the stop actually in force: the fixed --sl when
    # it is named, the coin's own ATR stop when --sl-atr sizes it.
    assert "stop_pct / 100.0 + a.maint_margin" in body, (
