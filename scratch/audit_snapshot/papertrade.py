...
def main() -> int:
    ...
    ap.add_argument("--book", default="", metavar="PATH",
                    help="state file for this book. The default is "
                         "data/paper.json; a second book (the beast) runs "
    ...
    _atr: dict[str, tuple[float, float]] = {}      # sym -> (atr%, when)
    ...
    def sym_lev(sym: str) -> float:
        ...
        room = a.sl / 100.0 + a.maint_margin
        if room > 0:
    ...
    def watch_live_orders() -> None:
        ...
        for pnd in list(resting):
            ...
            if st["status"] == "cancelled":
                # Cancelled by somebody else, or expired at the venue.
                resting.remove(pnd)
                cancel_resting(pnd, "cancelled at the exchange")
                continue
    ...
    if a.source == "break":
        ...
        print("filter: any colour but orange"
    ...
    while True:
      ...
      try:
          ...
          for pnd in list(resting):
              ...
              if a.entry == "edge":
                  ...
                  if used + margin > state["equity"] * a.max_exposure + 1e-9:
                      ...
                      continue
                  e = cross(pnd.sym, pnd.want, pnd.side)
                  sg = pnd.sig
          ...
          if sc:
              charts = list(charts) + [
                  (f"BITUNIX:{x['sym']}.P", "15", [x], {}) for x in sc]
          ...
          for tv_sym, res, sigs, ohlc in charts:
              ...
              for s in sigs:
                  ...
                  # way. Orange never appears at all; the indicator drops it
                  # before printing, so that half of the rule is free.
                  colour_ok = s["who"] != orange
                  ...
                  _atr = coin_atr(sym)
                  thin = (a.min_atr > 0 and _atr is not None
                          and _atr < a.min_atr)
                  ...
                  if (not colour_ok
                      ...
                      why = ("colour" if not colour_ok
                             ...
                             else (f"thin_coin: ATR {_atr:.2f}% of price, "
...
if __name__ == "__main__":
    try:
        ...
        print("\nstopped -- book kept in data/paper.json")
    finally:
        lk = Path(__file__).resolve().parent / "data" / "paper.lock"
        try:
