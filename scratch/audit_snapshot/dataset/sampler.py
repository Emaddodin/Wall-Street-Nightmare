...
def main() -> int:
    ...
    for chunk_start in range(0, a.n, a.chunk):
        ...
        Xs.append(X)
        ys.append((reason == 1).astype(np.int8))
        rs.append(reason.astype(np.int8))
        bs.append(bars.astype(np.int16))
        ps.append(boundary.astype(np.int8))
        total += n
        print(f"    {total}/{a.n}  ({time.time()-t0:.0f}s)", flush=True)

    X = np.concatenate(Xs)
    np.savez_compressed(X_path, X=X, y=np.concatenate(ys),
                        reason=np.concatenate(rs),
                        bars=np.concatenate(bs),
                        prov=np.concatenate(ps),
    ...
    manifest = {
        ...
        "outcomes": {"target": int((np.concatenate(ys) == 1).sum()),
                     "stop": int((np.concatenate(rs) == 2).sum()),
                     "open": int((np.concatenate(rs) == 0).sum())},
