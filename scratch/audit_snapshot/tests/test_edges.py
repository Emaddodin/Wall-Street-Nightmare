...
def test_a_wider_spread_gives_a_worse_fill_not_a_better_one(tmp_path):
    """A taker crosses the book; the simulation must not grant the mid."""
    fills = {}
    for bps in (2.0, 40.0):
    ...
    if len(fills) == 2:
        assert fills[40.0] > fills[2.0], "a wider spread produced a better fill"
