...
def test_edge_return_reads_the_adverse_move_for_both_sides():
    """The move back to the extreme is adverse on BOTH sides.

    This was the mirrored-answer bug: the SELL rows read fav, the
    favourable (down) move, so a SELL was judged on whether price fell
    toward its high.
    """
    bar = {"c": 100.0, "l": 99.0, "h": 101.0}
    moves = [0.5, 1.0]
    # BUY: the low is 1% below the close; a 1% down move returns there.
    assert entries.edge_return_bars(moves, "BUY", bar, 100.0) == 2
    # SELL: the high is 1% above the close; the same 1% up move returns.
    assert entries.edge_return_bars(moves, "SELL", bar, 100.0) == 2
    # A down-only path never returns a SELL to its high.
    assert entries.edge_return_bars([0.5, 0.4], "SELL", bar, 100.0) is None
    # A flat candle has no extreme worth returning to.
    assert entries.edge_return_bars(moves, "BUY", {"c": 100.0}, 100.0) is None
