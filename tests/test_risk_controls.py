...
# ------------------------------------------------------------- the spread
def test_the_spread_is_read_at_the_taker_fill_not_cached_forever(tmp_path):
    """A session that runs for days must not price a taker fill off a
    reading taken at start-up.

    The maker edge fill never touches the spread -- a resting limit fills
    AT the limit. The late fallback is a taker, and THAT fill must be
    priced off the spread as it is now, not as it was hours ago.
    """
    ...
    # Price parks away from the level: the band-edge order expires and the
    # fallback crosses the book at market, after the spread has widened.
    # One 900s sleep per cycle so the 8-bar window is 8 polls.
    b, _ = scenario(tmp_path, [LEVEL_UP * 1.05] * 12, exchange=ex,
                    extra_on_poll=widen,
                    argv=flags(interval=900, sprint=0),
                    polls=16)
    assert b.trades, f"no fallback fill: {b.log[-2:]}"
    assert reads["n"] >= 1, "the taker fill never asked for the spread"
    e = b.trades[0]["entry"]
    # 60 bps wide: the fill sits 0.3% past the last print. Priced off the
    # stale 2 bps it would sit 0.01% past it -- the midpoint (0.1%) draws
    # the line between "read now" and "believed forever".
    assert e > LEVEL_UP * 1.05 * (1 + 10 / 1e4), e
