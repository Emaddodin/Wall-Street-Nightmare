"""
tests/regime_journal_backtest_170d.py
======================================
Backtest and Per-Trade Journaling Engine across ALL 170 Trading Days
in the current market environment (2026-03-20 to 2026-09-20).
...
Exports:
- data/regime_trade_journal_170days.csv (Granular per-trade journal)
- data/regime_analytics_summary.json (Empirical win rates, session expectancies, trap matrix)
"""
...
def compute_lot_size(balance: float, max_lot_cap: float = 5.0) -> float:
    """To The Moon Dynamic Compounding Lot Ladder."""
    if balance < 200.0:
        return 0.05
    elif balance < 400.0:
        return 0.10
...
def run_170d_regime_backtest(
    ...
    if not candle_files:
        candle_files = sorted(Path(data_dir).glob("*.csv"))

    print(f"Loading and precomputing {len(candle_files)} trading days for Regime Audit...")
    days_data = [precompute_day_causal(f) for f in candle_files]
    days_data = [d for d in days_data if d is not None]
    print(f"Successfully processed {len(days_data)} valid daily market sessions.")
    ...
    for day_idx, d in enumerate(days_data, 1):
        ...
        for i in range(2, n):
            ...
            # -------------------------------------------------------------
            elif (curr_t - last_sig_time) >= 180.0 and balance >= 15.0:
                sig_dir = None
                sig_strat = None
                wick_r = 0.50
                ...
                if sig_dir:
                    ...
                    # Laya A+ boost multiplier on high wick ratio
                    laya_boost = 1.50 if wick_r >= 0.55 else 1.25 if wick_r >= 0.45 else 1.0
                    lot_vol = round(base_lot * laya_boost, 2)
    ...
    # Max Drawdown across 170 days
    max_dd = max(t.drawdown_pct for t in journal) if journal else 0.0

    analytics_summary = {
        "regime": "2026 High-Volatility Geopolitical Macro Regime",
    ...
    # Export CSV Journal
    csv_path = ROOT_DIR / "data" / "regime_trade_journal_170days.csv"
    ...
    print(f"📁 Saved complete 170-day per-trade journal ({total_trades} trades) to {csv_path}")

    # Export JSON Analytics Summary
    json_path = ROOT_DIR / "data" / "regime_analytics_summary.json"
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("      170-DAY MACRO REGIME AUDIT (2026-03-20 TO 2026-09-20)")
    print("=" * 80)
