...
def test_save_model_is_atomic_and_loads(tmp_path):
    """The artifact the books hot-reload must never be half-written."""
    out = tmp_path / "model.npz"
    got = S.save_model(str(out))
    assert got["n"] > 0 and 0.0 < got["threshold"] <= 1.0
    assert not Path(str(out) + ".tmp").exists()
    z = np.load(out)
    assert list(z["features"]) == S.FEATURES
    assert z["w"].shape == (2 * len(S.FEATURES),)  # raw + missing flags
    # and the book's own loader accepts it
    from filter_model import FilterModel
    fm = FilterModel(str(out))
    assert fm.n == got["n"] and abs(fm.threshold - got["threshold"]) < 1e-6


def test_save_model_overwrites_in_place_atomically(tmp_path):
    out = tmp_path / "model.npz"
    S.save_model(str(out))
    z1 = np.load(out)
    S.save_model(str(out))
    z2 = np.load(out)
    # same data, same seed -- the artifact is reproducible AND readable
    assert np.array_equal(z1["w"], z2["w"])
    assert not Path(str(out) + ".tmp").exists()
