...
def test_the_walk_orders_by_the_perchs_own_readiness():
    """The scout and the perch share one readiness formula: the ripest
    coin is visited first every sweep, before mere expansion."""
    coins = ["AAAUSDT", "BBBUSDT", "CCCUSDT"]
    exp = {"AAAUSDT": 0.6, "BBBUSDT": 3.0, "CCCUSDT": 1.0}
    # BBB is expanding hard, but AAA is one module from a print with the
    # tide behind it -- the eyesight goes to AAA first.
    ripe = {"AAAUSDT": S.readiness_of(
                {"short": 1, "side": 1, "tide": 1, "exp": 2.0}),
            "BBBUSDT": S.readiness_of(
                {"short": 5, "side": 1, "tide": -1, "exp": 3.0}),
            "CCCUSDT": S.readiness_of(
                {"short": 2, "side": 1, "tide": 1, "exp": 1.0})}
    order = S.walk_order(coins, [], exp, ripe)
    assert order[0] == "AAAUSDT", order
    assert order[1] == "CCCUSDT", order
    assert order[2] == "BBBUSDT", order
    # and without any ripe rows the old expansion ordering still rules
    order2 = S.walk_order(coins, [], exp)
    assert order2[0] == "BBBUSDT", order2
